...
| `enable_private_ca` | `true` | `private_ca` | Uses Google Managed Private CA (DevOps tier). If `false`, creates Google-managed public certificate using `managed_ssl_domain` |
...
# enable_private_ca     = false  # Use public managed certificate for production
