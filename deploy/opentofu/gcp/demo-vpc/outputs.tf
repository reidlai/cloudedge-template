...
output "demo_web_app_psc_service_attachment_self_link" {
  ...
  value       = local.enable_demo_web_app && local.enable_psc ? google_compute_service_attachment.demo_web_app_psc_attachment[0].id : null
}

output "demo_web_app_cloud_run_service_name" {
  description = "The name of the demo web app Cloud Run service."
  value       = local.enable_demo_web_app ? google_cloud_run_v2_service.demo_web_app[0].name : null
}

output "demo_web_app_backend_service_id" {
  description = "The ID of the demo web app backend service."
  value       = local.enable_demo_web_app ? google_compute_region_backend_service.demo_web_app_backend[0].id : null
}
